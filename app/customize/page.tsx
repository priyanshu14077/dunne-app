...
function HomeContent() {
  ...
  const [currentStep, setCurrentStep] = useState<"charms" | "base" | "space">(
    "charms"
  );
  ...
  const [previewedItem, setPreviewedItem] = useState<Product | Charm | null>(
    null
  );
  ...
  const [spacingMode, setSpacingMode] = useState<
    "standard" | "spaced" | "customize"
  >("standard");
  const [manualPlacedCharms, setManualPlacedCharms] = useState<
    PlacedCharmInstance[]
  >([]);
  const [note, setNote] = useState("");

  // Category state
  const [activeCharmCategory, setActiveCharmCategory] =
    useState("Travel & Wild");
  ...
  const handleCharmRemove = (instanceId: string) => {
    ...
    if (!instance) {
      ...
      const lastOfId = [...placedCharms]
        .reverse()
        .find((pc) => pc.charm.id === instanceId);
      if (lastOfId) {
        setPlacedCharmsSafe((prev) => {
    ...
    setPlacedCharmsSafe((prev) => {
      ...
      if (existingAtNew) {
        nextArr = prev.map((p) => {
          ...
          if (p.id === existingAtNew.id)
            return { ...p, anchorIndex: target.anchorIndex };
          return p;
        });
      } else {
        nextArr = prev.map((p) =>
          p.id === instanceId ? { ...p, anchorIndex: newIndex } : p
        );
      }
  ...
  const generateDesignId = () => {
    ...
    const random = crypto.randomUUID().split("-")[0]; // First 8 chars of UUID
  ...
  const handleAddToCart = async () => {
    ...
    placedCharms.slice(0, maxCharms).forEach((pc, idx) => {
      const n = idx + 1;
      baseProperties[`Charm ${n} Name`] = pc.charm.name;
      baseProperties[`Charm ${n} Anchor`] = pc.anchorIndex.toString(); // 0-8
      baseProperties[`Charm ${n} Quantity`] = "1";
    });
    ...
    // Add Charms: each with its own line item properties so anchor shows in cart
      ...
      if (pc.charm.handle || pc.charm.variantId) {
        items.push({
          ...
          properties: {
            "_Design ID": designId,
            Type: "Charm",
            "Anchor index": pc.anchorIndex.toString(), // 0-8, shown on charm in cart
            Position: pc.anchorIndex.toString(),
            "Linked To": selectedBase.name,
            "Charm name": pc.charm.name,
          },
        });
      }
    });
  ...
  const handleBack = () => {
    if (currentStep === "base") {
      ...
      setManualPlacedCharms((prev) =>
        prev.filter((pc) => isCharmLike(pc.charm))
      );
    } else if (currentStep === "space") {
      setCurrentStep("base");
    }
    ...
    <div className="flex flex-col h-[100dvh] bg-white font-sans text-slate-900 overflow-hidden pb-0">
      ...
      {showToast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[100] bg-[#DE3C27] text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-modal-slide-up select-none pointer-events-none lg:pointer-events-auto">
          ...
          >
            Maximum 9 charms reached
          </span>
          <button
            onClick={() => setShowToast(false)}
            className="hover:opacity-70 transition-opacity hidden lg:block"
          >
            <svg
              className="w-4 h-4"
              fill="none"
      ...
      <div className="flex-1 w-full bg-white relative min-h-0 flex items-center justify-center overflow-hidden">
        <JewelryCanvas
          ...
          previewCharm={
            previewedItem && "category" in previewedItem
              ? (previewedItem as Charm)
              : null
          }
          currentStep={currentStep}
      ...
      <div className="flex-shrink-0 h-auto min-h-[48px] py-1 w-full bg-[#1F4B30] relative flex items-center px-[30px]">
        ...
        {(currentStep === "base" ||
          currentStep === "space" ||
          (isTourActive && (tourStep === 3 || tourStep === 6))) && (
          <button
        ...
        <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center text-white text-center">
          <span
            className="text-[12px] opacity-90 mb-0.5"
        ...
        {(totalCharmCount > 0 ||
          (isTourActive && (tourStep === 1 || tourStep === 6))) && (
          <button
            id="walkthrough-next-arrow"
            onClick={handleNavigate}
            disabled={
              (currentStep === "base" && !selectedBase) ||
        ...
        {currentStep === "space" ? (
          <div className="w-full flex-1 flex flex-col gap-0">
            ...
            <div className="flex-1 flex flex-col items-center">
              <h3
                className="text-black text-[15px] font-normal mb-[10px] uppercase tracking-wider"
                style={{ fontFamily: "Neutra Text, sans-serif" }}
              >
                Add a Note
              </h3>
          ...
          <SelectionDrawer
            ...
            cardStates={
              currentStep === "charms" ? charmCardStates : baseCardStates
            }
            quantities={currentStep === "charms" ? charmQuantities : {}}
            onCardBodyClick={
              currentStep === "charms"
                ? handleCharmBodyClick
                : handleBaseBodyClick
            }
            onAdd={currentStep === "charms" ? handleCharmAdd : handleBaseAdd}
            onIncrement={
              currentStep === "charms" ? handleCharmIncrement : undefined
            }
            onRemove={
              currentStep === "charms" ? handleCharmRemove : handleBaseRemove
            }
            activeCategory={
              currentStep === "charms"
                ? activeCharmCategory
            ...
            onCategoryChange={
              currentStep === "charms"
                ? setActiveCharmCategory
                : setActiveBaseCategory
            }
      ...
      {isModalOpen && (
        <SelectedCharmsModal
          selectedCharms={[...placedCharms].sort(
      ...
      {isInfoModalOpen && (
        <InfoModal item={infoItem} onClose={() => setIsInfoModalOpen(false)} />
      )}
      <ShareModal
          ...
          <div className="bg-[#F4EFE6] w-full max-w-sm p-12 rounded-2xl shadow-2xl flex flex-col items-center justify-center text-center animate-scale-in">
            {isSuccess ? (
              <>
                ...
                <h3 className="text-2xl font-bold text-[#1F4B30] mb-2">
                  Added to Cart!
                </h3>
              ...
              <>
                ...
                <p className="text-black/60">
                  Linking your custom design with your cart.
                </p>
              </>
            )}
